...
async def test_opening_uses_ollama_when_enabled():
    ...
    line = await narrator.narrate_opening(Dice(seed=1))
...
async def test_room_uses_ollama_when_enabled():
    ...
    line = await narrator.narrate_room("The Sunken Crypt", room_type="crypt", rng=Dice(seed=1))
...
async def test_trap_falls_back_when_ollama_disabled(disabled_settings):
    narrator = Narrator(settings=disabled_settings)
    line = await narrator.narrate_trap("hidden spikes", triggered=True, rng=Dice(seed=1))
    assert isinstance(line, str)
    assert len(line) > 0
...
async def test_victory_uses_ollama_when_enabled():
    ...
    line = await narrator.narrate_victory(Dice(seed=1))
